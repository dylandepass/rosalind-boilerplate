...
export const ThemesSwitcher = memo(() => {
  const params = useParameter('theme', null) || {};

  function getThemeLinks(links) {
    if (!links?.themes) return [];

    return Object.keys(links.themes).map(brand => {
      const b = links.themes[brand];
      return {
        id: b.id,
        title: b.name,
        onClick: () => {
...
function setBrand(brand) {
  ...
  if(iframe) {
    const variablesLink = iframe.head.querySelector('#theme-variables');
    variablesLink.href = `./themes/${brand}.css`;
  }
}